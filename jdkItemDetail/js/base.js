...
var base = {
    //ctxpath:window.location.origin+proxy_url,
    ctxpath:"jdkItemDetail.jsp",
    key:"",
    token:'',
    doGet:function(json){
        $.ajax({
            ...
            url: base.ctxpath,
            data:json.data,
            //headers:{"sec":md5(asciiSort(json.data))},
...
//跳转京东客
function goJDKIos(url){
    window.webkit.messageHandlers.ZSTRequestGoJDK.postMessage(url);
}
function goJDKAndroid(url){
    window.ZSTRequestGoJDK.goJDKAndroid(url);
}
function goJDK(url){
    try{
            var type= judgeType();
...
function getAlertMsg(){
    try{
        ...
        if(type == 1){
            ...
            $("#alertImg").attr("src","http://XXXXXXX.png");
            $("#alertTxt").text("助商通");
        }
    }catch(e){
        console.log(e);
...
function ZSTObtainAlertMsg(data) {
    try{
        ...
        var _alertImg = base.isNvl(_data.alertImg,"http://XXXXXX/share/shareLogo.png"),
...
function shareLink(title,desc,img){
    ...
    (function(){
        $.ajax({
            ...
            dataType:"json",
            type:"get",
            url:"https://XXXXXX/api/voip/wechat/signature?url="+encodeURIComponent(encodeURIComponent(window.location.href)),
