		...
		<div v-show="list.length != 0" v-for="n in headList">
			...
			<ul class="bg-f fz15" >
				<router-link 
					...
					<img  src="@/assets/img/card2.png" v-if="item.cardType == 0"/>
					<img src="@/assets/img/card1.png" v-else-if="item.cardType == 1"/>
	    	...
	    	getData.getAgentCardList(_data).then(res=>{
				...
				if(_data.state == 1){
					...
					/*if(_len> 0){
					    ...
					    for(j;j<_len;j++){
					     	let _lastData ;
							_len == 1 ? _lastData = _data.data.cardorderInfoList[0].createDateStr :_lastData = _data.data.cardorderInfoList[_data.data.cardorderInfoList.length-1].createDateStr ;
							_self.headList = _self.headList.concat(_data.data.cardorderInfoListCount[j]);
 				          	if(_data.data.cardorderInfoListCount[j].createDateStr == _lastData){
 				             	break  ;
 			        		}
					    }
					}*/
