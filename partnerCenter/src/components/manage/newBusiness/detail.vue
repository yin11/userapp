<template>
	<div>
		...
		<div class="bg-f mb-5 fz14 fc-c pd-lr-10">
			...
			<p class="ico p-hide" v-if="cardcouponactiv.remark"><img src="@/assets/img/ico-zhe.png"/ >{{cardcouponactiv.remark || ''}}</p>
			<p class="ico p-hide" v-if="integralactive.remark"><img src="@/assets/img/ico-man.png"/>{{integralactive.remark || ''}}</p>
			<p class="ico p-hide" v-if="lotteryactive.remark"><img src="@/assets/img/ico-chong.png"/>{{lotteryactive.remark || ''}}</p>
		...
		<div class="bg-f mb-5 pd-lr-10 fz12">
			...
			<p class="clearfix count">
				...
				<router-link :to="{path:'/storeMenberCount',query:{shopId:shopId ,shopName:shopInfo.shopName}}" tag="span" class="fl-r fc-r fz14">查看统计数据</router-link>
			</p>
			...
			<p class="clearfix count">
				...
				<router-link :to="{path :'/storeCardCount' ,query:{shopId:shopId ,shopName:shopInfo.shopName}}" tag="span" class="fl-r fc-r fz14">查看统计数据</router-link>
			</p>
		...
		<div class="bg-f mb-5 pd-lr-10 fz12">
			...
			<p class="clearfix count">
				...
				<router-link :to="{path:'/storeProfit',query:{shopId:shopId ,shopName:shopInfo.shopName}}" tag="span" class="fl-r fc-r fz14">查看统计数据</router-link>
			</p>
			<div class="flex-box">
				<div class="item">
					<p>会员充值收益</p>
...
export default {
	...
	created: function(){
		let _self = this ;
		_self.coord= comm.getQueryString('coord') || '0,0';
	...
	methods: {
		...
		back(){
  		...
  		getStoreInfo(){
	    	...
	    	getData.getApplyAgentInfo(_data).then(res=>{
	    		let _data = res.data ;
				if(_data.state == 1){
					...
					_self.cardcouponactiv = _data.data.cardcouponactiv[0] ;
					_self.lotteryactive = _data.data.lotteryactive[0] ;
					_data.data.CardData.totalCardNum == 0 ? _self.activationRatio = 0 : _self.activationRatio = ((CardData.actvaNum / CardData.totalCardNum)*100).toFixed(2) ;
					_self.activeNum = Number(_data.data.integralactive.length) +Number(_data.data.cardcouponactiv.length) +Number(_data.data.lotteryactive.length) ;
				}else{
