		...
		<div class="bg-f mb-5">
			...
			<div class="clearfix bg-f fz15 title">
				<div class="fl-l flex-box fc-c">
					<div class="item">
						<input type="radio" name="type" id="type1" checked="checked" v-model="time" value="30" @change="selectTime"/>
			...
			<div class="relative">
				...
				<span class="fc-f total"><i v-if="time != ''">近{{time}}天</i><i v-else>全部</i>累计激活总数{{actvaNumCount}}</span>
			</div>
		</div>
		<!---->
		<router-link :to="{path:'/buyCardDetai',query:{shopId:shopId,shopName:shopName}}" tag="div" class="link relative">
			<p class="clearfix">
				<span class="fl-l">累计购卡总数(<i v-if="time != ''">近{{time}}天</i><i v-else>全部</i>)</span>
				<i class="right-arrow fl-r"></i>
				<span class="fl-r fw-6">{{totalCardNumCount}}</span>
			</p>
		</router-link>
	</div>
</template>
	...
	methods: {
	    ...
	    drawPie(){
   		...
   		getCardStatistics(){
   			...
   			getData.getStoreCardStatistics(_data).then(res=>{
   				let _data = res.data ;
				if(_data.state == 1){
					//得到数据，绘制图表
