		...
		<div class="bg-f mb-5">
			<div class="clearfix bg-f fz15 title">
				<div class="fl-l flex-box fc-c">
					<div class="item">
						<input type="radio" name="type" id="type1" checked="checked" @change="getNewShop" value="30" v-model="time"/>
			...
			<!--累计新增-->
			<p class="total fc-f fz12"><span v-show="time != ''">近{{time}}天</span><span v-show="time == ''">全部</span>累计新增商家{{totleCount.totalShopCount || 0}}家</p>
			<!--曲线图-->
			<div class="chart" id="myChart" :style="{width: '355px', height: '320px'}"></div>
			<div class="border-top">
				<p class="clearfix">
					<span class="fl-l"><span v-show="time != ''">近{{time}}天</span><span v-show="time == ''">全部</span>商家收益(元)</span>
...
export default {
	...
	methods: {
	    ...
	    getNewShop(){
	    	...
	    	getData.getNewShop(_data).then(res=>{
	    		let _data = res.data ;
				if(_data.state == 1){
