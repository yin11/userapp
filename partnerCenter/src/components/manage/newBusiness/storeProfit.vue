		...
		<div class="bg-f mb-5">
			...
			<div class="clearfix bg-f fz15 title">
				<div class="fl-l flex-box fc-c">
					...
					<div class="item">
						<input type="radio" name="type" id="type4" checked="checked"  @change="getProfitStatistics"  v-model="time" value=""/>
			...
			<div class="flex-box fz12 fc-c line-type">
				<div class="item checkbox-box" v-for="checkbox in selectList">
					<input type="checkbox" name="line" :id="'line'+checkbox.val" :value="checkbox.val" :checked="select.indexOf(checkbox.val)>=0" @click="selectType(checkbox.val)"/>
					<label class="checkbox ico-shadow" v-if="checkbox.val == 0" :for="'line'+checkbox.val">{{checkbox.name}}</label>
					<label class="checkbox ico-yellow" v-else-if="checkbox.val == 1" :for="'line'+checkbox.val">{{checkbox.name}}</label>
					<label class="checkbox ico-green" v-else-if="checkbox.val == 2" :for="'line'+checkbox.val">{{checkbox.name}}</label>
					<label class="checkbox ico-blue" v-else-if="checkbox.val == 3" :for="'line'+checkbox.val">{{checkbox.name}}</label>
				</div>
...
export default {
	...
	methods: {
	    ...
	    getProfitStatistics(){
	    	...
	    	getData.getStoreProfitStatistics(_data).then(res=>{
	    		let _data = res.data ;
				if(_data.state == 1){
					...
					_data.data.memberEstimateProfitList.forEach((i,n)=>{ //购物预估收益
						_arry4 = _arry4.concat(i.memberCount);
					});
