	...
	<div>
		...
		<header class="bg-f " :style="'padding-top:'+status_bar_height+'px'">
			...
			<div class="search-box bg-f" v-show="searchState"> 
				<form action="javascript:return true;">
					<input type="search bg-f" placeholder="请输入职员姓名或电话" @keyup.13="searchStaff" v-model="nameOrPhone" ref="input1"/>
		...
		<ul class="bg-f ">
			<div class="pd-lr-10 pd-tb-5 bg-c fz15 clearfix">
				<form action="javascript:return true;">
					<input class="search-input" type="search" placeholder="请输入职员姓名或电话" @keyup.13="searchStaff" v-model="nameOrPhone" ref="input1"/>
				</form>
				<p class="clearfix">
					<span class="fl-l">当前职员{{count}}人</span>
					<span class="fl-r">最多可激活{{count-promoteCount}}人</span>
				</p>
			</div>
			...
			<div v-else v-for="item in list">
				<router-link :to="{path:'/staffDetail',query:{id: item.id}}" tag="div" class="relative border-buttom list-con"  >
					...
					<p class="clearfix ">
						<span class="fl-l fz17">{{item.realName}}<i class="ico" :class="item.gender == 0 ? 'ico-girl' : 'ico-man' "></i></span>
						<span class="fl-r fc-c fz14" v-if="item.state == 0">已注销</span>
					</p>
					<p class="clearfix">
						<a :href="'tel:' + item.phone" class="fc-c fz14">手机号码：{{item.phone}}<i class="ico ico-phone"></i></a>
						<!--<button type="button" class="fl-r btn-quit bg-r fc-f">离职</button>-->
					</p>
...
export default {
	...
	methods: {
		...
		searchStaff(){
			...
			if(comm.rule.empty(_self.nameOrPhone,"搜索内容不能为空")){
				_self.pageNum = 1 ;
				//_self.count = 0 ;
				_self.getAgentStaff();
			}
	    ...
	    getAgentStaff(){
	   		...
	   		getData.getAgentStaff(_data).then(res=>{
	   			let _data = res.data ;
				if(_data.state == 1){
					...
					_data.data.agStaffs.forEach(function(i,n){
						i.entryTime = comm.timestampToTime(i.createTime) ; // 将入职时间戳转为时间
					});
	    ...
	    //员工离职
	    setStaffState(id,state,promoteState){
	    	let _self = this ;
	    	let _data = {
	    		token : comm.TOKEN ,
	    		id : id ,
