<template>
	<div>
		...
		<header class="bg-f" :style="'padding-top:'+status_bar_height+'px'">
			...
			<span class="icon fc-r" @click="subFormate" v-if="disable">提交</span>
			<span class="icon fc-c" v-else>提交</span>
		</header>
		...
		<div>
			<ul class="fz14 info-list mb-7 bg-f ">
				...
				<li class="clearfix">
					...
					<span class="fl-r">{{userinfo.contacts || ''}}</span>
				</li>
				<li class="clearfix">
					<span class="fl-l fc-c">收货人手机</span>
					<span class="fl-r">{{userinfo.contactPhone || ''}}</span>
				</li>
				<li class="clearfix">
					<span class="fl-l fc-c">收货人地址</span>
					<span class="fl-r contactway p-hide">{{userinfo.contactWay || ''}}</span>
				</li>
			</ul>
		</div>
		<!--门店照片-->
