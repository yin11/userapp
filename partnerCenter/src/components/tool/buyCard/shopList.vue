...
export default {
	...
	methods: {
	    ...
	    getShopList(){
	    	...
	    	getData.getAgentStoreList(_data).then(res=>{
	    		let _data = res.data ;
				if(_data.state == 1){
	   ...
	   goBuyCard(id,contacts,receiverPhone,receiverAddress,shopName){
	   		this.$router.push({path:'/buyCard',query:{id:id,name:contacts,phone:receiverPhone,address:receiverAddress,shopName:shopName}});
	   }
    },
    mounted () {  
   		let _self = this ;
