	...
	<div>
		...
		<div class="bg-f list-box">
			...
			<div>
				...
				<div class="flex-box fz14">
					<div class="item">
						<input type="radio" name="shouyi" id="shouyi1" value="1" v-model="type" :checked="type == 1" @change="selectList"/>
						<label for="shouyi1" >已结算</label>
					</div>
					<div class="item">
						<input type="radio" name="shouyi" id="shouyi2" value="2" v-model="type" :checked="type == 2" @change="selectList"/>
			...
			<div>
				<!--已结算-->
				<div >
					<div class="nullImg" v-if="titleList.length == 0" ><img src="@/assets/img/null.png" width="50%"/></div>
					<div else>
						<ul v-for="con in titleList" >
							<div class="bg-c fz10 con-title">
								<p class="fw-6">{{con.transDateStr}}</p>
								<p class="fc-c">共{{con.totalOrder}}笔收入    <span v-show="type == 1">已结算</span><span v-show="type == 2">预估</span>收入￥{{con.transMoney}}</p>
							</div>
							<li v-for="item in list "  class="mb-5 bg-f list-con" v-if="item.transTimeStr.indexOf(con.transDateStr)>=0">
								...
								<div v-if="item.platform == 1 || item.platform == 2 || item.platform == 9">
									<p class="fc-c fz9">店铺名称：{{item.shopName || ''}}</p>
									<p class="fc-c fz9">创建时间：{{item.createTimeStr }}</p>
									<p class="fc-c fz9">结算时间：{{item.transTimeStr}}</p>
								...
								<div class="relative fz10 fc-c" v-else>
									<p>直接收益：{{item.shopName || ''}}</p>
									<p>充值时间：{{item.transTimeStr }}</p>
									<div class="sum-box">
										<p>收益金额</p>
...
export default {
	...
	methods:{
		...
		getList(){
			...
			getData.getZstAmounts(_data).then(res=>{
				let _data = res.data ;
				if(_data.state == 1){
					...
					_data.data.forEach((n)=>{
						n.createTimeStr = comm.timestampToTime(n.createTime) ;
						n.transTimeStr = comm.timestampToTime(n.transTime) ; //将时间戳转为正确格式
					});
		...
		getPredictList(){
			...
			getData.getPreincomeAmounts(_data).then(res=>{
				let _data = res.data ;
				if(_data.state == 1){
					if(_data.data.length < _self.limit){ //获取的数据小于每页数量，已到达最后一页
						_self.scrollState = false ; //禁止滚动加载
