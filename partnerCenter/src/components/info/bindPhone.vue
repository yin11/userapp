		...
		<div class="main">
			...
			<div class="bg-f form-box" v-if="formState">
				<div class="border-buttom ico-phone">
					<input type="number" placeholder="请输入原绑定手机号" maxlength="11" pattern="[0-9]*" class="fz16" v-model="usedPhone" readonly="readonly"/>
				</div>
...
export default {
	...
	methods:{
        ...
        getNewCode(){
       		let _self = this ;
       		if(comm.rule.empty(_self.newPhone , '手机号不能为空') && comm.rule.phone(_self.newPhone) ){
       			_self.sendNewMess();
       			let data = {
        ...
        testUsed(){
        	...
        	let data = {
       		...
       		if( comm.rule.empty(_self.usedCode,'验证码不能为空')){
       			getData.getTestCode(data).then(res=>{
	        		let _data = res.data ;
					if(_data.state == 1){
						//验证旧手机号成功，跳转输入新手机号
        ...
        testNew(){
        	...
        	let data = {
       		...
       		if(comm.rule.empty(_self.newPhone,'手机号不能为空') && comm.rule.phone(_self.newPhone) && comm.rule.empty(_self.newCode,'验证码不能为空')){
       			getData.getTestCode(data).then(res=>{
	        		let _data = res.data ;
					if(_data.state == 1){
						//绑定新手机号成功，跳转首页
